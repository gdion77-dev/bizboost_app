# app.py
# Bizboost - Εξωδικαστικός (Streamlit + Postgres + PDF)
# - Νέα πρόβλεψη, Αποθήκευση
# - Προβολή/Επεξεργασία υπόθεσης σε ίδια σελίδα ή νέο παράθυρο (?view=case&case_id=...)
# - Καταχώριση πραγματικής ρύθμισης, Σύγκριση πρόβλεψης vs πραγματικής
# - Διαγραφή υπόθεσης
...
try:
    ...
    if chosen:
        ...
        st.toast("⚠️ Δεν βρέθηκε TTF (NotoSans/NotoSerif). Θα χρησιμοποιηθεί Helvetica.")
except Exception:
    PDF_FONT = "Helvetica"
...
def make_pdf(case_dict:dict)->bytes:
    ...
    def _cm_list_to_points(widths_cm, doc_):
        ...
        total = sum(pts); avail = _available_width(doc_)
    ...
    def _personalized_reasoning(cd):
        ...
        if secured_cnt: parts.append("Εξασφαλισμένες οφειλές: κατώφλι εξασφάλισης (security floor).")
        if other_cnt:
    ...
    if debts:
        ...
        tt = Table(rows, colWidths=[w*cm for w in widths_cm], repeatRows=1)
    ...
    story.append(Paragraph(_personalized_reasoning(case_dict), styles["P"])); story.append(Spacer(1, 12))
...
def show_case_detail(df_all: pd.DataFrame, case_id: str):
    ...
    if rowdf.empty:
        st.error("Η υπόθεση δεν βρέθηκε."); return
    row = rowdf.iloc[0].to_dict()

    # Top bar with Back
    back_col, pdf_col = st.columns([1,1])
    with back_col:
        if st.button("⬅️ Back", key=f"back_{case_id}", use_container_width=True):
            st.session_state.open_case_id = None
            st.rerun()
    with pdf_col:
        if st.button("⬇️ Export PDF", key=f"pdf_{case_id}", use_container_width=True):
            try:
            ...
            case_for_pdf = {
                ...
                "debtor_age": row.get("debtor_age"),
                "adults": row.get("adults"),
                "children": row.get("children"),
                "monthly_income": row.get("monthly_income"),
                "edd_household": row.get("edd_manual") if row.get("edd_use_manual") else None,
                "extras_sum": (row.get("extra_medical") or 0)+(row.get("extra_students") or 0)+(row.get("extra_legal") or 0),
                "avail": None,  # δεν αποθηκεύουμε συγκεντρωτικά· προαιρετικό
            ...
            st.download_button(
                "Λήψη PDF",
                data=make_pdf(case_for_pdf),
                file_name=f"{row.get('case_id')}_prediction.pdf",
    ...
    cA, cB = st.columns(2)
    with cA:
        st.metric("Ηλικία", s(row.get("debtor_age","")))
        st.metric("Μηνιαίο εισόδημα", eur(row.get("monthly_income")))
        st.metric("Ακίνητη περιουσία", eur(row.get("property_value")))
    with cB:
        st.metric("ΕΔΔ (αν έχει)", eur(row.get("edd_manual") if row.get("edd_use_manual") else 0))
        st.metric("Ιατρικά/Σπουδές/Δικαστικά", eur((row.get("extra_medical") or 0)+(row.get("extra_students") or 0)+(row.get("extra_legal") or 0)))
        st.metric("Ημερομηνία", s(row.get("predicted_at","")))
    ...
    # Real settlement form (with prefill if exists)
    try:
        real_saved = json.loads(row.get("real_debts_json") or "[]")
    except Exception:
        real_saved = []
    ...
    for i, d in enumerate(debts):
        pre = None
        for r in (real_saved or []):
            if r.get("creditor")==d.get("creditor") and r.get("loan_type")==d.get("loan_type") and _to_float(r.get("balance"))==_to_float(d.get("balance")):
                pre = r; break
        with st.expander(f"Οφειλή #{i+1} – {d.get('creditor','')} / {d.get('loan_type','')} / Υπόλοιπο: {eur_num(d.get('balance'))} €", expanded=True):
            col1,col2,col3,col4 = st.columns(4)
            real_term    = col1.number_input("Πραγμ. μήνες", 0, 1200, int(pre.get("real_term_months") or 0) if pre else 0, key=f"detail_rt_{case_id}_{i}")
            real_monthly = col2.number_input("Πραγμ. δόση (€)", 0.0, 1e9, float(pre.get("real_monthly") or 0.0) if pre else 0.0, step=10.0, key=f"detail_rm_{case_id}_{i}")
            real_write   = col3.number_input("Διαγραφή (€)", 0.0, float(d.get("balance") or 0.0), float(pre.get("real_writeoff") or 0.0) if pre else 0.0, step=100.0, key=f"detail_rw_{case_id}_{i}")
    ...
    with cS1:
        if st.button("💾 Αποθήκευση πραγματικής ρύθμισης", type="primary", key=f"save_real_{case_id}", use_container_width=True):
            row_update = row.copy()
            ...
            st.rerun()
    with cS2:
        if st.button("✖️ Cancel", key=f"cancel_{case_id}", use_container_width=True):
            st.session_state.open_case_id = None
            st.rerun()

    # Comparison section
    try:
        real_after = json.loads(load_data_db()[load_data_db()["case_id"]==case_id].iloc[0].get("real_debts_json") or "[]")
    except Exception:
        real_after = real_saved

    if real_after:
        st.subheader("Σύγκριση Πρόβλεψης vs Πραγματικής")
        def key_of(x): return f"{x.get('creditor','')}|{x.get('loan_type','')}|{round(_to_float(x.get('balance')),2)}"
        real_map = {key_of(r): r for r in (real_after or [])}
        rows_cmp = []
        for d in debts:
            ...
            rows_cmp.append({
                ...
                "Πρόβλ. Υπόλ. Ρύθμ. (€)": eur_num(d.get("predicted_residual",0)),
                ...
                "Διαφορά Δόσης (€)": eur_num((_to_float(r.get("real_monthly")) - _to_float(d.get("predicted_monthly"))) if r and r.get("real_monthly") is not None else 0.0),
                "Διαφορά Διαγραφής (€)": eur_num((_to_float(r.get("real_writeoff")) - _to_float(d.get("predicted_writeoff"))) if r and r.get("real_writeoff") is not None else 0.0),
                "Διαφορά Υπολοίπου (€)": eur_num((_to_float(r.get("real_residual")) - _to_float(d.get("predicted_residual"))) if r and r.get("real_residual") is not None else 0.0),
            })
...
# Direct open via query param (?view=case&case_id=...)
try:
...
if page == "Νέα Πρόβλεψη":
    ...
    if submitted:
        ...
        case_for_pdf = {
            ...
            "extras_sum": float(extras_sum), "avail": float(avail), "property_value": float(property_value),
            "debts": debts_to_store, "predicted_at": now_str
        }
        st.download_button("⬇️ Λήψη Πρόβλεψης (PDF)", data=make_pdf(case_for_pdf), file_name=f"{case_id}_prediction.pdf", mime="application/pdf", use_container_width=True)
...
# ───────────────────────── Page: Cases & Real ─────────────────────────
    ...
    cols_per_row = 3
    for i in range(0, len(dfv), cols_per_row):
        row_chunk = dfv.iloc[i:i+cols_per_row]
        ccols = st.columns(len(row_chunk))
        for (idx, (_, rc)) in enumerate(row_chunk.iterrows()):
            with ccols[idx]:
                cid = rc["case_id"]
                st.markdown(f"**{rc['borrower'] or '—'}**")
                st.caption(f"Υπόθεση: `{cid}`  \nΗμερ.: {rc['predicted_at'] or '—'}")
                ...
                with c1:
                    if st.button("📂 Άνοιγμα", key=f"open_{cid}", use_container_width=True):
                        ...
                        st.rerun()
                with c2:
                    st.markdown(f"[↗︎ Νέο παράθυρο](:/?)", help="Φόρτωσε σε νέο tab και βάλε ?view=case&case_id=...")
                    # Πλήρες URL με query:
                    st.write(f"`?view=case&case_id={cid}`")
